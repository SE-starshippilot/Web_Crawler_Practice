...
# 从网页爬取数据
def scraping(baseURL, save_html_folder):
    # 获得全部页面信息存入列表中
    board_content_list = []
    for offset in range(10):
        ...
        time.sleep(0.5)
        if save_html_folder:
            with open(f'{save_html_folder}/offset_{offset}.html', 'w+', encoding='utf-8') as f:
...
# 处理每个电影的信息
def process_metadata(metadta, download_image=True):
    movie_attrs = {
...
# 从每个网页中找到电影信息
def batch_retrive_metadata(board_content_list, download_image=True):
    # 从页面上爬取每个电影的信息
    ...
    processed_movie_metadata = [process_metadata(_, download_image=True) for _ in raw_movie_metadata]
    return processed_movie_metadata


# 将爬取到的数据存入Excel文件中
def build_excel_file(data, file_name):
    # 创建一个Excel文件
...
def config_parser():
    ...
    parser.add_argument('--download_image', '-d',type=bool, default=True, help='download image')
    parser.add_argument('--save_html_folder', '-s', type=str, default='./pages', help='If specified, save html to this folder.')
    parser.add_argument('--excel_file_name', '-n', type=str, default='猫眼电影榜单Top100.xls', help='excel file name')
...
def main():
    ...
    print("<---------------数据爬取完成--------------->")
    print("<---------------开始处理数据--------------->")
    data = batch_retrive_metadata(board_html, args.download_image)
    print("<---------------数据处理完成--------------->")
    print("<---------------开始存储数据--------------->")
    print("将爬取数据存入Excel文件：")
    build_excel_file(data, args.excel_file_name)
    print("<---------------数据存储完成--------------->")
    print("################程序运行结束################")
